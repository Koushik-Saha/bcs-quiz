...
import com.example.varianttecnology.kotlinmultiplechoicequiz.Interface.IAnswerSelect
    ...
    override fun selectedAnswer(): CurrentQuestion {
        ...
        if (Common.answerSheetList[questionIndex].type == Common.ANSWER_TYPE.NO_ANSWER)
        {
            ...
            if (Common.selected_values.size > 1) //IF multiple choice
            {
                ...
                //Here we will get first character of answer
                //Ex: arr[0] = A.Paris
                //      arr[1] = B.New York
                //      CorrectAnswer = A,B
                //      So this code will get 'A' and 'B' in array for us

                for (i in arrayAnswer!!.indices)
                    if (i < arrayAnswer!!.size - 1)
                        result.append(StringBuffer((arrayAnswer!![i] as String).substring(0,1)).append(","))
                    else
